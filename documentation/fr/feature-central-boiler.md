...
2. dans la configuration des VTherms, vous indiquez si le VTherm doit contrôler la chaudière. En effet, dans une installation hétérogène, certains VTherm doivent commander la chaudière et d'autres non. Vous devez donc indiquer dans la configuration de chaque VTherm s'il contrôle la chaudière ou pas.
...
4. dès que le nombre d'équipements pilotés par le VTherm demandant du chauffage (i.e. son `hvac_action` passe à `Heating`) dépasse un seuil paramétrable, alors le `binary_sensor.central_boiler` passe à `on` et **si un service d'activation a été configuré, alors ce service est appelé**.
...
6. vous avez accès à deux entités :
   ...
   - une de type `sensor` nommé par défaut `sensor.nb_device_active_for_boiler`, donne le nombre d'équipements qui demande du chauffage. Par exemple, un VTherm ayant 4 vannes dont 3 demandes du chauffage fera passer ce capteur à 3. Seuls les équipements des VTherms qui sont marqués pour contrôler la chaudière centrale sont comptabilisés.
...
4. pour certains services, vous aurez besoin d'un paramètre. Cela peut être le 'Mode CVC' `climate.set_hvac_mode` ou la température cible pour `climate.set_temperature`. Ce paramètre doit être configuré sous la forme `attribut:valeur` en fin de chaîne.

Exemples (à adapter à votre cas) :
...
- `switch.pompe_chaudiere/switch.turn_on` : pour allumer le switch qui alimente la pompe de la chaudière,
- `switch.pompe_chaudiere/switch.turn_off` : pour allumer le switch qui alimente la pompe de la chaudière,
- ...

### Comment trouver la bonne action ?
Pour trouver l'action à utiliser, le mieux est d'aller dans "Outils de développement / Actions", de chercher l'action à appeler, l'entité à commander et l'éventuel paramètre à donner.
